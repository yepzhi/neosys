/* ═══════════════════════════════════════════
   NEOSYS AEON — App Logic v5.4.2 FINAL STABLE
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // HD Poster Download (v5.4.2 restored)
...
function renderOutreach() {
    ...
    // Recovery for visibility: add 'loaded' class with staggered delay
    setTimeout(() => {
        const cards = grid.querySelectorAll('.outreach-card');
        cards.forEach((card, index) => {
            setTimeout(() => {
                card.classList.add('loaded');
            }, index * 40); // 40ms stagger for smooth entry
        });
    }, 100);
