const translations = {
    es: {
        // Nav
        nav_mandamientos: "Principios",
        nav_marco: "Marco Conceptual",
        nav_whitepaper: "Framework",
        nav_acercate: "Acércate",
        nav_comunidad: "Comunidad",
        nav_unete: "Únete",

        // Hero
        hero_tagline: "Framework basado en sistemas para la integración de ciencia, tecnología y toma de decisiones humana.",
        hero_subtitle: "Construido sobre evidencia, modelos adaptativos y síntesis interdisciplinaria.",
        hero_btn_mandamientos: "Principios Operativos",
        hero_btn_whitepaper: "Ver Framework (v4.3)",
        hero_explore: "Explorar",

        postulado_label: "Base Epistémica (A)",
        postulado_title_new: "El <span class=\"text-accent\">método científico</span> es una herramienta para construir <span class=\"text-accent\">modelos predictivos</span> y verificables de la realidad.",
        postulado_desc_new: "No protege hipótesis: las somete a falsación constante.<br>No depende de intuiciones: depende de datos empíricos.<br><br>Es la arquitectura de la eficiencia tecnológica y social moderna.",

        // Marco Conceptual (Anteriormente Manifiesto)
        manifesto_label: "Filosofía Conceptual (B)",
        manifesto_title: "La información no es conocimiento.<br><span class='text-accent'>El conocimiento es aplicación sistémica.</span>",
        manifesto_p1: "✨ La ciencia permite el modelado del entorno. La ingeniería permite su optimización. La tecnología permite el escalamiento. NEOSYS propone una arquitectura de pensamiento para reducir la entropía en la toma de decisiones.",
        manifesto_p2: "La síntesis sin aplicación carece de impacto. La ejecución sin modelado genera error sistémico. El framework integra ambos vectores en un bucle de retroalimentación continuo.",
        manifesto_p3: "NEOSYS no es una ideología. Es un modelo de análisis interdisciplinario — los 10 <strong>Principios Operativos</strong> — orientados a la optimización de procesos mediante evidencia y lógica bayesiana.",

        // 10 Principios Operativos
        mand_label: "Heurísticas del Sistema (C)",
        mand_title: "Los 10 Principios<br><span class='text-accent'>del Cosmos</span>",
        mand_desc: "Guías fundamentales para la generación, validación y aplicación de conocimiento.",
        poster_download_btn: "Descargar Framework de Principios (HD) ✨",

        m1_title: "I. HONRARÁS LA VERDAD POR ENCIMA DE LA COMODIDAD.",
        m1_body: "No adorarás la certeza falsa; buscarás lo real, aunque incomode.",
        m2_title: "II. CULTIVARÁS LA CURIOSIDAD COMO PRÁCTICA ESENCIAL.",
        m2_body: "Preguntarás antes de afirmar, y explorarás antes de juzgar.",
        m3_title: "III. RECORDARÁS QUE ERES POLVO DE ESTRELLAS.",
        m3_body: "No por pequeñez, sino por pertenencia: eres universo con conciencia.",
        m4_title: "IV. DEFENDERÁS LA EVIDENCIA COMO BRÚJULA.",
        m4_body: "No por dogma, sino porque es el filtro más confiable que tenemos.",
        m5_title: "V. PRACTICARÁS LA HUMILDAD EPISTÉMICA.",
        m5_body: "Frente a la inmensidad, rechazarás el ego y abrazarás el asombro.",
        m6_title: "VI. CUIDARÁS LA TIERRA COMO TU ÚNICO HOGAR COMPROBADO.",
        m6_body: "No hay otro cercano ni confirmado; protegerlo es un acto de razón y responsabilidad.",
        m7_title: "VII. AMARÁS LA VIDA DONDE EXISTA, Y LA RESPETARÁS DONDE SE ENCUENTRE.",
        m7_body: "Toda forma de vida merece dignidad, atención y cuidado.",
        m8_title: "VIII. NO CONVERTIRÁS EL MISTERIO EN IGNORANCIA.",
        m8_body: "Lo desconocido no es magia; es una invitación a comprender e investigar.",
        m9_title: "IX. NO DARÁS LUGAR A LA PSEUDOCIENCIA NI AL ENGAÑO DISFRAZADO.",
        m9_body: "Cuestionarás afirmaciones extraordinarias y exigirás evidencia proporcional.",
        m10_title: "X. COMPARTIRÁS LO QUE SABES Y CONSTRUIRÁS CON ELLO.",
        m10_body: "El conocimiento guardado se pierde; el conocimiento aplicado transforma.",

        // Capas de Análisis
        prob_label: "Capa de Aplicación",
        prob_title: "Optimización de la relación<br><span class='text-accent'>Ciencia-Tecnología-Sociedad</span>",
        prob1_title: "✨ Fase 1: Análisis Científico",
        prob1_body: "Priorización de modelos predictivos sobre sesgos cognitivos y narrativa ideológica.",
        prob2_title: "🔗 Fase 2: Sostenibilidad Sistémica",
        prob2_body: "Integración de una ética de la responsabilidad basada en el reconocimiento del entorno operativo viable.",
        prob3_title: "🔬 Fase 3: Ejecución Industrial",
        prob3_body: "Sincronización de la validación académica con la implementación técnica para reducir latencias innovadoras.",
        prob4_title: "📉 Fase 4: Optimización de Valor",
        prob4_body: "Toma de decisiones bajo incertidumbre mediante el uso de lógica bayesiana y modelado probabilístico.",

        // STEM section
        stem_label: "Arquitectura de Progreso",
        stem_title: "El bucle de generación de valor.<br><span class='text-accent'>Modelado, Aplicación y Escalamiento.</span>",
        stem_desc: "El método científico es el protocolo más eficiente para la validación de información. Su impacto se maximiza mediante el bucle STEM.",
        stem_s1_title: "Ciencia",
        stem_s1_body: "Genera modelos",
        stem_s2_title: "Ingeniería",
        stem_s2_body: "Optimiza modelos",
        stem_s3_title: "Tecnología",
        stem_s3_body: "Materializa modelos",
        stem_s4_title: "Economía",
        stem_s4_body: "Escala el impacto",
        stem_p1: "La medición es la base del conocimiento.",
        stem_p2: "La experimentación valida el progreso.",
        stem_p3: "La ingeniería permite la transferencia.",
        stem_p4: "La escalabilidad garantiza el impacto social.",

        // Distinción
        dist_label: "Definición y Alcance",
        dist_title: "Taxonomía de NEOSYS.<br><span class='text-accent'>Propuesta y límites.</span>",
        dist_is_header: "✨ NEOSYS ES:",
        dist_is_1: "<strong>Framework Interdisciplinario:</strong> integración de múltiples áreas del saber tecnológico.",
        dist_is_2: "<strong>Heurístico:</strong> proporciona guías de acción adaptativas basadas en el entorno.",
        dist_is_3: "<strong>Data-Driven:</strong> prioriza la evidencia empírica sobre la narrativa.",
        dist_is_4: "<strong>Agnóstico:</strong> independiente de estructuras políticas o dogmáticas.",
        dist_is_5: "<strong>Infraestructura Abierta:</strong> disponible para su implementación descentralizada.",
        dist_is_6: "<strong>Analítico:</strong> enfocado en la optimización de procesos complejos.",
        dist_not_header: "✕ NEOSYS NO ES:",
        dist_not_1: "No es una <strong>religión</strong> o propuesta de metafísica.",
        dist_not_2: "No es una <strong>ideología cerrada</strong> o movimiento social.",
        dist_not_3: "No es una <strong>organización jerárquica</strong> o centralizada.",
        dist_not_4: "No es un <strong>sistema de creencias</strong> absoluto o inamovible.",
        dist_not_5: "No <strong>sustituye</strong> a la ciencia; actúa como su capa de integración.",
        dist_quote: "\"La evidencia es el terreno más sólido para la toma de decisiones.<br>Un modelo que no genera predicciones útiles es irrelevante.\"",

        wp_label: "Documentación Técnica",
        wp_title: "Framework Whitepaper<br><span class='text-accent'>v4.3 (Technical Draft)</span> <span style='display:inline-block; animation: sparkle-pulse 3s ease-in-out infinite;'>✨</span>",
        wp_desc: "Marco de referencia conceptual para la integración de sistemas de conocimiento técnico.",
        wp_btn: "Descargar Framework PDF",

        join_label: "Fase de Implementación",
        join_title: "Integración de la Red<br><span class='text-accent'>de Implementación</span>",
        join_desc: "La adopción del framework es descentralizada. Implemente las heurísticas en su entorno y sincronice con la red de nodos.",
        join_badge_title: "Registrar Nodo (v4.3) ✨",
        join_badge_desc: "Incorpore su perfil al directorio de implementación técnica y verifique su compromiso con la metodología sistémica.",
        join_placeholder: "Nombre completo / Identificador",
        join_email: "Correo institucional/referencia",
        join_phone: "Enlace de contacto (opcional)",
        join_social: "ID de Red Profesional (opcional)",
        join_country_default: "Región Operativa",
        join_btn_generate: "Generar y Validar Nodo",
        join_btn_save: "Sincronizar",
        join_btn_share: "Difundir",
        join_share_text: "Implementando el Framework NEOSYS #SystemsThinking #EvidenceBased — Hacia una toma de decisiones basada en datos. https://yepzhi.com/neosys/",
        join_upload_text: "Cargar Identificación Visual",
        join_badge_member: "NODO DE IMPLEMENTACIÓN",
        join_badge_tagline: "NEOSYS: Optimización de sistemas mediante el método científico y lógica probabilística.",

        con_label: "Transferencia de Conocimiento",
        con_title: "NEOSYS define el marco sistémico.<br><span class='text-accent'>JovenesSTEM ejecuta la formación.</span>",
        con_p1: "Los principios operativos de NEOSYS —indagación metódica y transferencia técnica— constituyen el núcleo del programa <strong>JovenesSTEM</strong> y su arquitectura pedagógica, el <strong>BlueBook</strong>.",
        con_p2: "Mientras NEOSYS proporciona el framework de análisis, JovenesSTEM materializa la formación de la nueva generación de especialistas en ciencia y tecnología.",
        con_btn: "🚀 Explorar JovenesSTEM",
        con_node1: "Framework Sistémico",
        con_node2: "Programa de Formación",
        con_node3: "Arquitectura BlueBook",

        out_label: "Recursos de Verificación",
        out_title: "Fuentes de <span class='text-accent'>Validación</span>",
        out_desc: "Acceso a bibliotecas y canales de referencia para la profundización en el método científico y la ingeniería.",
        out_btn_explore: "Ver Recursos →",
        outreach_categories: [
            {
                id: "youtube",
                title: "Canales de Referencia",
                items: [
                    { name: "Veritasium", topic: "Ciencia General", link: "https://www.youtube.com/c/veritasium", icon: "🌌" },
                    { name: "Kurzgesagt", topic: "Ciencia General", link: "https://www.youtube.com/c/inanutshell", icon: "🦆" },
                    { name: "Mark Rober", topic: "Ingeniería", link: "https://www.youtube.com/c/MarkRober", icon: "🛠️" },
                    { name: "MinutePhysics", topic: "Física", link: "https://www.youtube.com/user/minutephysics", icon: "⏱️" },
                    { name: "Fireship", topic: "Sistemas/Programación", link: "https://www.youtube.com/c/Fireship", icon: "💻" }
                ]
            },
            {
                id: "series",
                title: "Series Documentales",
                items: [
                    { name: "Cosmos (Carl Sagan)", topic: "Astronomía/Epistemología", link: "https://en.wikipedia.org/wiki/Cosmos:_A_Personal_Voyage", icon: "📺" },
                    { name: "Cosmos (Neil deGrasse Tyson)", topic: "Astronomía/Epistemología", link: "https://en.wikipedia.org/wiki/Cosmos:_A_Spacetime_Odyssey", icon: "📺" },
                    { name: "Star Trek: Discovery", topic: "Ficción Científica", link: "https://intl.startrek.com/shows/star-trek-discovery", icon: "🚀" }
                ]
            },
            {
                id: "books",
                title: "Bibliografía Técnica",
                items: [
                    { name: "The BlueBook", topic: "Currículo STEM", link: "https://yepzhi.com/jovenesstem/", icon: "📘" },
                    { name: "Astrophysics for People in a Hurry", topic: "Astrofísica", link: "https://www.amazon.com/Astrophysics-People-Hurry-deGrasse-Tyson/dp/0393609391", icon: "📖" }
                ]
            }
        ],

        // Comunidad
        comm_label: "Nodos Activos",
        comm_title: "Red de Implementación",
        comm_desc: "Directorio de perfiles que han adoptado el framework NEOSYS para la optimización de sus procesos de decisión.",
        comm_tab_directory: "Lista de Nodos",
        comm_tab_map: "Mapa de Impacto",
        comm_join_btn: "✨ Registrarse como Nodo",
        join_privacy_note: "La información de perfil y ubicación se integra al mapa global del framework. Datos sensibles permanecen encriptados.",
        join_city: "Ubicación",
        join_badge_desc2: "Propuesta técnica para la transición hacia una sociedad basada en evidencia y sistemas abiertos.",

        // Footer
        footer_quote: "NEOSYS no garantiza certezas finales, sino un proceso de aproximación probabilística cada vez más preciso. Su eficacia depende de la rigurosidad en su aplicación.",
        footer_copy: "Framework de Sistemas Abiertos. Sin jerarquías centrales. Sin barreras de acceso."
    },

    en: {
        // Nav
        nav_mandamientos: "Principles",
        nav_marco: "Conceptual Framework",
        nav_whitepaper: "Framework",
        nav_acercate: "Acercate",
        nav_comunidad: "Community",
        nav_unete: "Join",

        // Hero
        hero_tagline: "Systems-based framework for integrating science, technology, and human decision-making.",
        hero_subtitle: "Built on evidence, adaptive models, and interdisciplinary synthesis.",
        hero_btn_mandamientos: "Operational Principles",
        hero_btn_whitepaper: "View Framework (v4.3)",
        hero_explore: "Explore",

        postulado_label: "Epistemic Basis (A)",
        postulado_title_new: "The <span class=\"text-accent\">scientific method</span> is a tool for building <span class=\"text-accent\">predictive models</span> and verifiable interpretations of reality.",
        postulado_desc_new: "It does not protect hypotheses: it subjects them to constant falsification.<br>It does not depend on intuition: it depends on empirical data.<br><br>It is the base architecture of modern technological and social efficiency.",

        // Conceptual Framework (Formerly Manifesto)
        manifesto_label: "Conceptual Philosophy (B)",
        manifesto_title: "Information is not knowledge.<br><span class='text-accent'>Knowledge is systemic application.</span>",
        manifesto_p1: "✨ Science allows for environmental modeling. Engineering allows for its optimization. Technology allows for scaling. NEOSYS proposes a thought architecture to reduce entropy in decision-making.",
        manifesto_p2: "Synthesis without application lacks impact. Execution without modeling generates systemic error. The framework integrates both vectors in a continuous feedback loop.",
        manifesto_p3: "NEOSYS is not an ideology. It is an interdisciplinary analysis framework — the 10 <strong>Operational Principles</strong> — aimed at optimizing processes through evidence and Bayesian logic.",

        // 10 Operational Principles
        mand_label: "System Heuristics (C)",
        mand_title: "The 10 Principles<br><span class='text-accent'>of the Cosmos</span>",
        mand_desc: "Fundamental guidelines for the generation, validation, and application of knowledge.",
        poster_download_btn: "Download Principles Framework (HD) ✨",

        m1_title: "I. HONOR TRUTH ABOVE CONVENIENCE.",
        m1_body: "Do not worship false certainty; seek the real, even if it is uncomfortable.",
        m2_title: "II. CULTIVATE CURIOSITY AS AN ESSENTIAL PRACTICE.",
        m2_body: "Inquire before affirming, and explore before judging.",
        m3_title: "III. REMEMBER THAT YOU ARE STARDUST.",
        m3_body: "Not for smallness, but for belonging: you are a conscious universe.",
        m4_title: "IV. DEFEND EVIDENCE AS YOUR COMPASS.",
        m4_body: "Not out of dogma, but because it is the most reliable filter we have.",
        m5_title: "V. PRACTICE EPISTEMIC HUMILITY.",
        m5_body: "Facing the vastness, reject the ego and embrace wonder.",
        m6_title: "VI. CARE FOR THE EARTH AS YOUR ONLY PROVEN HOME.",
        m6_body: "There is no other confirmed nearby; protecting it is an act of reason.",
        m7_title: "VII. LOVE LIFE WHERE IT EXISTS, AND RESPECT IT WHERE FOUND.",
        m7_body: "Every life form deserves dignity, attention, and care.",
        m8_title: "VIII. DO NOT TURN MYSTERY INTO IGNORANCE.",
        m8_body: "The unknown is not magic; it's an invitation to understand and investigate.",
        m9_title: "IX. GIVE NO ROOM TO PSEUDOSCIENCE OR DISGUISED DECEPTION.",
        m9_body: "Question extraordinary claims and demand proportional evidence.",
        m10_title: "X. SHARE WHAT YOU KNOW AND BUILD WITH IT.",
        m10_body: "Hoarded knowledge is lost; applied knowledge transforms.",

        // Application Layers
        prob_label: "Application Layer",
        prob_title: "Optimizing the relationship between<br><span class='text-accent'>Science, Technology, and Society</span>",
        prob1_title: "✨ Phase 1: Scientific Analysis",
        prob1_body: "Prioritizing predictive models over cognitive biases and ideological narratives.",
        prob2_title: "🔗 Phase 2: Systemic Sustainability",
        prob2_body: "Integrating an ethics of responsibility based on the recognition of the viable operative environment.",
        prob3_title: "🔬 Phase 3: Industrial Execution",
        prob3_body: "Synchronizing academic validation with technical implementation to reduce innovation latency.",
        prob4_title: "📉 Phase 4: Value Optimization",
        prob4_body: "Decision-making under uncertainty through the use of Bayesian logic and probabilistic modeling.",

        // STEM Architecture
        stem_label: "Progress Architecture",
        stem_title: "The value generation loop.<br><span class='text-accent'>Modeling, Application, and Scaling.</span>",
        stem_desc: "The scientific method is the most efficient protocol for information validation. Its impact is maximized through the STEM loop.",
        stem_s1_title: "Science",
        stem_s1_body: "Generates models",
        stem_s2_title: "Engineering",
        stem_s2_body: "Optimizes models",
        stem_s3_title: "Technology",
        stem_s3_body: "Materializes models",
        stem_s4_title: "Economy",
        stem_s4_body: "Scales impact",
        stem_p1: "Measurement is the foundation of knowledge.",
        stem_p2: "Experimentation validates progress.",
        stem_p3: "Engineering enables technical transfer.",
        stem_p4: "Scalability guarantees social impact.",

        // Distinction
        dist_label: "System Definition",
        dist_title: "Scope of NEOSYS.<br><span class='text-accent'>Proposal and Boundaries.</span>",
        dist_is_header: "✨ NEOSYS IS:",
        dist_is_1: "<strong>Interdisciplinary Framework:</strong> integration of multiple fields of knowledge.",
        dist_is_2: "<strong>Heuristic:</strong> provides adaptive action guidelines.",
        dist_is_3: "<strong>Data-Driven:</strong> prioritizes empirical evidence.",
        dist_is_4: "<strong>Agnostic:</strong> independent of political or dogmatic structures.",
        dist_is_5: "<strong>Open:</strong> available for free implementation.",
        dist_is_6: "<strong>Analytical:</strong> focused on complex problem solving.",
        dist_not_header: "✕ NEOSYS IS NOT:",
        dist_not_1: "It is not a <strong>religion</strong> or pseudoscientific system.",
        dist_not_2: "It is not a <strong>closed ideology</strong> or political movement.",
        dist_not_3: "It is not a <strong>hierarchical organization</strong>.",
        dist_not_4: "It is not an <strong>immovable belief system</strong>.",
        dist_not_5: "It does not <strong>replace</strong> science; it facilitates its social integration.",
        dist_quote: "\"Evidence is the most solid ground for decision-making.<br>A model that generates no useful predictions is irrelevant.\"",

        wp_label: "Technical Documentation",
        wp_title: "Framework Whitepaper<br><span class='text-accent'>v4.3 (Technical Draft)</span> <span style='display:inline-block; animation: sparkle-pulse 3s ease-in-out infinite;'>✨</span>",
        wp_desc: "Conceptual reference framework for integrated knowledge systems.",
        wp_btn: "Download Framework PDF",

        join_label: "Implementation",
        join_title: "Network Integration<br><span class='text-accent'>#NeosysAeon</span>",
        join_desc: "Framework adoption is decentralized. Implement operational principles in your environment and sync with the global node network.",
        join_badge_title: "Register Node (Badge) ✨",
        join_badge_desc: "Incorporate your profile into the systemic implementation directory and verify your commitment to evidence-based methodology.",
        join_placeholder: "Full Name",
        join_email: "Institutional/Personal Email",
        join_phone: "Contact (optional)",
        join_social: "Social Media ID (optional)",
        join_country_default: "Geographic Region",
        join_btn_generate: "Generate & Validate",
        join_btn_save: "Synchronize",
        join_btn_share: "Spread",
        join_share_text: "Implementing NEOSYS Framework #SystemsThinking #EvidenceBased — Towards data-driven decision making. https://yepzhi.com/neosys/",
        join_upload_text: "Load Visual ID",
        join_badge_member: "IMPLEMENTATION NODE",
        join_badge_tagline: "NEOSYS: System optimization through scientific method and probabilistic logic.",

        con_label: "Knowledge Transfer",
        con_title: "NEOSYS defines the systemic frame.<br><span class='text-accent'>JovenesSTEM executes the training.</span>",
        con_p1: "NEOSYS operational principles —methodical inquiry and technical transfer— form the core of the <strong>JovenesSTEM</strong> program and its pedagogical architecture, the <strong>BlueBook</strong>.",
        con_p2: "While NEOSYS provides the analysis framework, JovenesSTEM materializes the training of the next generation of STEM specialists.",
        con_btn: "🚀 Explore JovenesSTEM",
        con_node1: "Systemic Framework",
        con_node2: "Training Program",
        con_node3: "BlueBook Architecture",

        out_label: "Verification Resources",
        out_title: "Validation <span class='text-accent'>Sources</span>",
        out_desc: "Access reference libraries and channels for deep dives into scientific method and engineering.",
        out_btn_explore: "View Resources →",
        outreach_categories: [
            {
                id: "youtube",
                title: "Reference Channels",
                items: [
                    { name: "Veritasium", topic: "General Science", link: "https://www.youtube.com/c/veritasium", icon: "🌌" },
                    { name: "Kurzgesagt", topic: "General Science", link: "https://www.youtube.com/c/inanutshell", icon: "🦆" },
                    { name: "Mark Rober", topic: "Engineering", link: "https://www.youtube.com/c/MarkRober", icon: "🛠️" },
                    { name: "MinutePhysics", topic: "Physics", link: "https://www.youtube.com/user/minutephysics", icon: "⏱️" },
                    { name: "Fireship", topic: "Programming", link: "https://www.youtube.com/c/Fireship", icon: "💻" }
                ]
            },
            {
                id: "series",
                title: "Documentaries",
                items: [
                    { name: "Cosmos (Carl Sagan)", topic: "Astronomy", link: "https://en.wikipedia.org/wiki/Cosmos:_A_Personal_Voyage", icon: "📺" },
                    { name: "Cosmos (Neil deGrasse Tyson)", topic: "Astronomy", link: "https://en.wikipedia.org/wiki/Cosmos:_A_Spacetime_Odyssey", icon: "📺" },
                    { name: "Star Trek: Discovery", topic: "Science Fiction", link: "https://intl.startrek.com/shows/star-trek-discovery", icon: "🚀" }
                ]
            },
            {
                id: "books",
                title: "Technical Bibliography",
                items: [
                    { name: "The BlueBook", topic: "STEM Curriculum", link: "https://yepzhi.com/jovenesstem/", icon: "📘" },
                    { name: "Astrophysics for People in a Hurry", topic: "Astrophysics", link: "https://www.amazon.com/Astrophysics-People-Hurry-deGrasse-Tyson/dp/0393609391", icon: "📖" }
                ]
            }
        ],

        // Community
        comm_label: "Active Nodes",
        comm_title: "Implementation Network",
        comm_desc: "Directory of profiles that have adopted the NEOSYS framework for process optimization.",
        comm_tab_directory: "Node List",
        comm_tab_map: "Impact Map",
        comm_join_btn: "✨ Register as Node",
        join_privacy_note: "Profile and location data are integrated into the global framework map. Sensitive data remains encrypted.",
        join_city: "Location",
        join_badge_desc2: "Technical proposal for the transition towards an evidence-based society and open systems.",

        // Footer
        footer_quote: "NEOSYS does not guarantee final certainties, but an increasingly precise probabilistic approximation process. Its efficacy depends on the rigor of its application.",
        footer_copy: "Open Systems Framework. No central hierarchies. No entry barriers."
    },

    cn: {
        // Nav
        nav_mandamientos: "原则",
        nav_marco: "框架",
        nav_whitepaper: "白皮书",
        nav_acercate: "深入了解",
        nav_comunidad: "社区",
        nav_unete: "加入",

        // Hero
        hero_tagline: "基于系统的科学、技术与决策集成框架。",
        hero_subtitle: "构建于可验证的证据、自适应模型和跨学科综合之上。",
        hero_btn_mandamientos: "操作原则",
        hero_btn_whitepaper: "查看框架 (v4.3)",
        hero_explore: "探索",

        postulado_label: "认识论基础 (A)",
        postulado_title_new: "<span class=\"text-accent\">科学方法</span> 是构建 <span class=\"text-accent\">预测模型</span> 和可验证解释的工具。",
        postulado_desc_new: "它不保护假设：它对假设进行持续的证伪测试。<br>它不依赖直觉：它依赖经验数据。<br><br>它是现代技术和社会效率的基础架构。",

        // 概念框架 (原宣言)
        manifesto_label: "概念哲学 (B)",
        manifesto_title: "资讯不等于知识。<br><span class='text-accent'>知识是系统化的应用。</span>",
        manifesto_p1: "✨ 科学实现了环境建模。工程实现了环境优化。技术实现了规模化。NEOSYS 提议一种旨在减少决策熵的思维架构。",
        manifesto_p2: "没有应用的综合缺乏影响力。没有建模的执行会产生系统误差。框架将这两个向量集成在持续的反馈循环中。",
        manifesto_p3: "NEOSYS 不是一种意识形态。它是一个跨学科分析模型——10 <strong>操作原则</strong>——旨在通过证据和贝叶斯逻辑优化流程。",

        // 10 操作原则
        mand_label: "系统启发式 (C)",
        mand_title: "宇宙 10 原则<br><span class='text-accent'>框架核心</span>",
        mand_desc: "知识生成、验证和应用的基本指南。",
        poster_download_btn: "下载原则框架 (HD) ✨",

        m1_title: "I. 尊崇真理高于舒适。",
        m1_body: "不崇拜虚假的确定性；追求真实，即使它令人不安。",
        m2_title: "II. 培养好奇心作为核心实践。",
        m2_body: "在肯定之前询问，在判断之前探索。",
        m3_title: "III. 记住你是星尘。",
        m3_body: "并非因为渺小，而是因为归属：你是拥有意识的宇宙。",
        m4_title: "IV. 捍卫证据作为指南针。",
        m4_body: "不因教条，而因它是我们拥有的最可靠的过滤器。",
        m5_title: "V. 践行认识论上的谦逊。",
        m5_body: "面对浩瀚，拒绝自负，拥抱惊奇。",
        m6_title: "VI. 像对待唯一证实的家园一样爱护地球。",
        m6_body: "没有其他确认的邻居；保护地球是理性与责任的行为。",
        m7_title: "VII. 热爱生命存在的每一处，尊重生命被发现的每一处。",
        m7_body: "所有的生命形式都值得尊严、关注和关爱。",
        m8_title: "VIII. 不要将神秘转化为无知。",
        m8_body: "未知并非魔法；它是理解与研究的邀请。",
        m9_title: "IX. 不给伪科学或变相欺骗留有余地。",
        m9_body: "质疑非凡的主张，并要求比例相当的证据。",
        m10_title: "X. 分享你所知道的，并以此进行建设。",
        m10_body: "被珍藏的知识会丢失；被应用的知识会转化。",

        // 分析层
        prob_label: "应用层",
        prob_title: "优化<br><span class='text-accent'>科学-技术-社会</span>之间的关系",
        prob1_title: "✨ 第一阶段：科学分析",
        prob1_body: "优先考虑预测模型而非认知偏差和意识形态叙事。",
        prob2_title: "🔗 第二阶段：系统可持续性",
        prob2_body: "整合基于对可行操作环境认知的责任伦理。",
        prob3_title: "🔬 第三阶段：工业执行",
        prob3_body: "同步学术验证与技术实施，以减少创新延迟。",
        prob4_title: "📉 第四阶段：价值优化",
        prob4_body: "使用贝叶斯逻辑和概率建模优化不确定性下的决策。",

        // STEM 架构
        stem_label: "进步架构",
        stem_title: "价值生成循环。<br><span class='text-accent'>建模、应用与扩展。</span>",
        stem_desc: "科学方法是资讯验证最有效的协议。其影响力通过 STEM 循环实现最大化。",
        stem_s1_title: "科学",
        stem_s1_body: "生成模型",
        stem_s2_title: "工程",
        stem_s2_body: "优化模型",
        stem_s3_title: "技术",
        stem_s3_body: "实现模型",
        stem_s4_title: "经济",
        stem_s4_body: "扩展影响",
        stem_p1: "测量是知识的基础。",
        stem_p2: "实验验证进步。",
        stem_p3: "工程实现技术转移。",
        stem_p4: "扩展性保证社会影响力。",

        // 区别
        dist_label: "系统定义",
        dist_title: "NEOSYS 的范围。<br><span class='text-accent'>提案与边界。</span>",
        dist_is_header: "✨ NEOSYS 是：",
        dist_is_1: "<strong>跨学科框架：</strong>多个知识领域的整合。",
        dist_is_2: "<strong>启发式的：</strong>提供自适应行动指南。",
        dist_is_3: "<strong>数据驱动的：</strong>优先考虑经验证据。",
        dist_is_4: "<strong>不可知论的：</strong>独立于政治或教条结构。",
        dist_is_5: "<strong>开放的：</strong>可供自由实施。",
        dist_is_6: "<strong>分析性的：</strong>专注于复杂问题的解决。",
        dist_not_header: "✕ NEOSYS 不是：",
        dist_not_1: "它不是<strong>宗教</strong>或伪科学体系。",
        dist_not_2: "它不是<strong>封闭的意识形态</strong>或政治运动。",
        dist_not_3: "它不是<strong>等级化组织</strong>。",
        dist_not_4: "它不是<strong>不可改变的信仰体系</strong>。",
        dist_not_5: "它不<strong>取代</strong>科学；它促进科学的社会集成。",
        dist_quote: "\"证据是决策最坚实的基础。<br>不产生有用预测的模型是无关紧要的。\"",

        wp_label: "技术文档",
        wp_title: "框架白皮书<br><span class='text-accent'>v4.3 (技术初稿)</span> <span style='display:inline-block; animation: sparkle-pulse 3s ease-in-out infinite;'>✨</span>",
        wp_desc: "集成知识系统的概念参考框架。",
        wp_btn: "下载白皮书 PDF",

        join_label: "实施",
        join_title: "网络集成<br><span class='text-accent'>#NeosysAeon</span>",
        join_desc: "框架采用是去中心化的。在您的环境中实施操作原则并与全球节点网络同步。",
        join_badge_title: "注册节点 (徽章) ✨",
        join_badge_desc: "将您的个人资料纳入系统实施目录，并验证您对基于证据的方法论的承诺。",
        join_placeholder: "姓名",
        join_email: "机构/个人电子邮件",
        join_phone: "联系方式 (可选)",
        join_social: "社交媒体 ID (可选)",
        join_country_default: "地理区域",
        join_btn_generate: "生成并验证",
        join_btn_save: "同步",
        join_btn_share: "传播",
        join_share_text: "实施 NEOSYS 框架 #系统思维 #证据驱动 — 迈向数据驱动决策。 https://yepzhi.com/neosys/",
        join_upload_text: "加载视觉标识",
        join_badge_member: "实施节点",
        join_badge_tagline: "NEOSYS：通过科学方法和概率逻辑优化系统。",

        con_label: "知识转移",
        con_title: "NEOSYS 定义系统化框架。<br><span class='text-accent'>JovenesSTEM 执行培训。</span>",
        con_p1: "NEOSYS 的操作原则——有条理的查询和技术转移——构成了 <strong>JovenesSTEM</strong> 计划及其教学架构 <strong>BlueBook</strong> 的核心。",
        con_p2: "在 NEOSYS 提供分析框架的同时，JovenesSTEM 实现了下一代 STEM 专家的培训。",
        con_btn: "🚀 探索 JovenesSTEM",
        con_node1: "系统化框架",
        con_node2: "培训计划",
        con_node3: "BlueBook 架构",

        out_label: "验证资源",
        out_title: "验证<span class='text-accent'>来源</span>",
        out_desc: "访问参考库和渠道，深入研究科学方法和工程。",
        out_btn_explore: "查看资源 →",
        outreach_categories: [
            {
                id: "youtube",
                title: "参考渠道",
                items: [
                    { name: "Veritasium", topic: "一般科学", link: "https://www.youtube.com/c/veritasium", icon: "🌌" },
                    { name: "Kurzgesagt", topic: "一般科学", link: "https://www.youtube.com/c/inanutshell", icon: "🦆" },
                    { name: "Mark Rober", topic: "工程", link: "https://www.youtube.com/c/MarkRober", icon: "🛠️" },
                    { name: "MinutePhysics", topic: "物理", link: "https://www.youtube.com/user/minutephysics", icon: "⏱️" },
                    { name: "Fireship", topic: "编程", link: "https://www.youtube.com/c/Fireship", icon: "💻" }
                ]
            },
            {
                id: "series",
                title: "纪录片",
                items: [
                    { name: "Cosmos (Carl Sagan)", topic: "天文学", link: "https://en.wikipedia.org/wiki/Cosmos:_A_Personal_Voyage", icon: "📺" },
                    { name: "Cosmos (Neil deGrasse Tyson)", topic: "天文学", link: "https://en.wikipedia.org/wiki/Cosmos:_A_Spacetime_Odyssey", icon: "📺" },
                    { name: "Star Trek: Discovery", topic: "科幻", link: "https://intl.startrek.com/shows/star-trek-discovery", icon: "🚀" }
                ]
            },
            {
                id: "books",
                title: "技术书目",
                items: [
                    { name: "The BlueBook", topic: "STEM 课程", link: "https://yepzhi.com/jovenesstem/", icon: "📘" },
                    { name: "Astrophysics for People in a Hurry", topic: "天体物理", link: "https://www.amazon.com/Astrophysics-People-Hurry-deGrasse-Tyson/dp/0393609391", icon: "📖" }
                ]
            }
        ],

        // 社区
        comm_label: "活动节点",
        comm_title: "实施网络",
        comm_desc: "采用 NEOSYS 框架进行流程优化的个人资料目录。",
        comm_tab_directory: "节点列表",
        comm_tab_map: "影响力地图",
        comm_join_btn: "✨ 注册为节点",
        join_privacy_note: "个人资料和位置数据集成到全球框架地图中。敏感数据保持加密。",
        join_city: "位置",
        join_badge_desc2: "迈向基于证据的社会和开放系统的技术提案。",

        // Footer
        footer_quote: "NEOSYS 不保证最终的确定性，而是保证一个日益精确的概率近似过程。其效力取决于其实施的严谨性。",
        footer_copy: "开放系统框架。无中央等级。无准入门槛。"
    }
};
